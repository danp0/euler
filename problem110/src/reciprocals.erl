...
factor_up_to(1, _UpTo) ->
  [{1,1}];
factor_up_to(N, UpTo) ->
  factor(N, UpTo, 0, []).

find(GT) ->
  Start = product(primes(25)),
  find(Start, Start, GT).

find(N, Incr, GT) ->
  Solutions = solutions_by_factors(N),
  case Solutions > GT of
    true ->
      N;
    false ->
      find(N+Incr, Incr, GT)
...
%%        case Solutions > 400 of
%%          true ->
%%            io:format("~p: ~p~n", [I, Solutions]),
%%            io:format("    ~p~n", [factor(I)]);
%%          false ->
%%            ok
%%        end
%%    end,
%%    lists:seq(N, N+1000000, N)),
  ok = stop(),
  ok.
...
product_test_() ->
  [
   ?_assertEqual(1, product([1])),
   ?_assertEqual(2, product([1,2])),
